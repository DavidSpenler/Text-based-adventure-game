...
vector<action> scenario::add_variances(vector<action> a) {
	...
	for (unsigned int i=0;i<a.size();++i) {
		...
		while (start != -1) {
			...
			new_actions = {};
			for (unsigned int x=0;x<cache_actions.size();++x) {
...
string scenario::print_inventory() {
	...
	for (it_type iterator=inventory.begin();iterator != inventory.end();iterator++) {
		if (conditions["has_"+iterator->first] == true) {
			...
			cout << item+string(20-iterator->first.size(),' ');
			++line_num;
		}
	}
	return "";
...
string scenario::take_action(string action) {
	...
	string core_action;
	bool matched = false;
	for (unsigned int i=0;i<action_words.size();++i) {
		matched = false;
		for (unsigned int i2=0;i2<synonyms.size();++i2) {
			for (unsigned int i3=0;i3<synonyms[i2].size();++i3) {
		...
		if (i == 0 && not matched) {
			return "Dungeon Quest\tScore: "+to_string(score)+"/48\n\n'"+action_words[0]+"' not recognized. Type 'help for assistance.'";
	...
	for (unsigned int i=0;i<valid_actions.size();++i) {
...
bool scenario::check_match(const string& action,const string& match) {
	...
	for (unsigned int z=0;z<words_largest.size();++z) {
